...
class User(Base):
    ...
    conversations: Mapped[list["Conversation"]] = relationship("Conversation", back_populates="owner", foreign_keys="Conversation.owner_id")
    team_memberships: Mapped[list["TeamMember"]] = relationship("TeamMember", back_populates="user", cascade="all, delete-orphan", foreign_keys="TeamMember.user_id")
...
class Team(Base):
    ...
    conversations: Mapped[list["Conversation"]] = relationship("Conversation", back_populates="team", foreign_keys="Conversation.team_id")
...
class Conversation(Base):
    ...
    team_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("teams.id", ondelete="SET NULL"), nullable=True, index=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False, default=utcnow)
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False, default=utcnow, onupdate=utcnow)
    ended_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)

    owner: Mapped["User | None"] = relationship("User", back_populates="conversations", foreign_keys=[owner_id])
    team: Mapped["Team | None"] = relationship("Team", back_populates="conversations", foreign_keys=[team_id])
    speakers: Mapped[list["Speaker"]] = relationship("Speaker", back_populates="conversation", cascade="all, delete-orphan")
    utterances: Mapped[list["Utterance"]] = relationship("Utterance", back_populates="conversation", cascade="all, delete-orphan")
...
class Speaker(Base):
    ...
    utterances: Mapped[list["Utterance"]] = relationship("Utterance", back_populates="speaker", cascade="all, delete-orphan")
...
class Utterance(Base):
    ...
    __table_args__ = (Index("ix_utterances_conversation_seq", "conversation_id", "sequence_number"),)

    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    conversation_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("conversations.id", ondelete="CASCADE"), nullable=False, index=True)
    speaker_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("speakers.id", ondelete="SET NULL"), nullable=True, index=True)
...
class Document(Base):
    ...
    uploaded_by: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id", ondelete="SET NULL"), nullable=True, index=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False, default=utcnow)
